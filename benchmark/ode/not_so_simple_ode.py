...
class yeast_glycolysis(ODE):
    ...
    def initial_conditions(self, noise=True, rng=random):
        noise = int(noise)
        return [max(0, rng.uniform(up, low) + rng.gauss(0, sigma)) for (up, low), sigma in zip(self.range, self.sigma)]
...
class double_pendulum(ODE):
    ...
    def ode(self, m, l, g):
        def dy(y, t):
            ...
            temp = m*l**2
            denom = (16. - 9 * np.cos(Dphi)**2)

            dphi1 = 6./temp * (2.*p1 - 3.*np.cos(Dphi) * p2) / denom
            dphi2 = 6./temp * (2.*p2 - 3.*np.cos(Dphi) * p1) / denom
